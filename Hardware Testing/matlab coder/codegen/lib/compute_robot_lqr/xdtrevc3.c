...
void xdtrevc3(const double T[144], double vr[144])
{
  ...
  int j;
  int ki;
  memset(&work[0], 0, 36U * sizeof(double));
  x[0] = 0.0;
  x[1] = 0.0;
  x[2] = 0.0;
  x[3] = 0.0;
  ...
  for (ki = 11; ki >= 0; ki--) {
    if (ip == -1) {
      ...
      wi = 0.0;
      if (ip != 0) {
        wi = sqrt(fabs(T[ki + 12 * (ki - 1)])) * sqrt(fabs(T[iyend - 1]));
      }
      smin = fmax(2.2204460492503131E-16 * (fabs(wr) + wi),
      ...
      if (ip == 0) {
            ...
            if (b_j + 1 == 1) {
              ...
              if (T[iyend] == 0.0) {
                ...
                scale = xdlaln2(2, 1, smin, T, iyend, work, b_j + 24, wr, 0.0,
                                x, &smax);
                if ((smax > 1.0) && (fmax(work[b_j - 1], work[b_j]) >
                ...
                if (scale != 1.0) {
                  ...
                  for (j = 25; j <= b_vectorUB; j += 2) {
                    r = _mm_loadu_pd(&work[j - 1]);
                    _mm_storeu_pd(&work[j - 1],
                                  _mm_mul_pd(_mm_set1_pd(scale), r));
                  }
                  for (j = vectorUB; j <= ii; j++) {
                    work[j - 1] *= scale;
                  }
                }
                work[b_j + 23] = x[0];
                work[b_j + 24] = x[1];
                xaxpy(b_j - 1, -x[0], T, b_i + 1, work);
                xaxpy(b_j - 1, -x[1], T, b_j * 12 + 1, work);
                b_j -= 2;
              }
            }
            if (guard1) {
        ...
        if (ki > 0) {
          xgemv(ki, work, work[ki + 24], vr, ki * 12 + 1);
        }
        iyend = ki * 12;
        ii = 0;
        smax = fabs(vr[iyend]);
        for (j = 0; j < 11; j++) {
          scale = fabs(vr[(iyend + j) + 1]);
          if (scale > smax) {
            ii = j + 1;
            smax = scale;
          }
        }
        smax = 1.0 / fabs(vr[ii + 12 * ki]);
        iyend = ki * 12;
        ii = iyend + 12;
        vectorUB = ((ii - iyend) / 2 * 2 + iyend) + 1;
        b_vectorUB = vectorUB - 2;
        for (j = iyend + 1; j <= b_vectorUB; j += 2) {
          ...
          if (b_j + 1 >= 1) {
            ...
            if (b_j + 1 == 1) {
              ...
              if (T[iyend] == 0.0) {
                ...
                xaxpy(b_j - 1, -x[2], T, b_i + 1, work);
                xaxpy(b_j - 1, -x[3], T, b_j * 12 + 1, work);
                b_j -= 2;
              }
            }
            if (guard1) {
              ...
              if ((b_j >= 1) && (!(-x[0] == 0.0))) {
                ...
                for (j = vectorUB; j < b_j; j++) {
                  work[j + 12] += -x[0] * T[ii + j];
                }
              }
              xaxpy(b_j, -x[2], T, b_j * 12 + 1, work);
              b_j--;
            }
          } else {
            exitg1 = 1;
        ...
        if (ki + 1 > 2) {
          ...
          if (smax != 1.0) {
            if (smax == 0.0) {
              if (ix0 + 1 <= iyend) {
                memset(&vr[ix0], 0,
                       (unsigned int)(iyend - ix0) * sizeof(double));
              }
            } else {
              ii = ((iyend - ix0) / 2 * 2 + ix0) + 1;
              vectorUB = ii - 2;
              for (j = ix0 + 1; j <= vectorUB; j += 2) {
                r = _mm_loadu_pd(&vr[j - 1]);
                r = _mm_mul_pd(_mm_set1_pd(smax), r);
                _mm_storeu_pd(&vr[j - 1], r);
              }
          ...
          xgemv(ki - 1, work, work[ki + 24], vr, ki * 12 + 1);
        } else {
          ii = ix0 + 12;
          vectorUB = ((ii - ix0) / 2 * 2 + ix0) + 1;
          b_vectorUB = vectorUB - 2;
          for (j = ix0 + 1; j <= b_vectorUB; j += 2) {
            r = _mm_loadu_pd(&vr[j - 1]);
        ...
        smax = 0.0;
        for (j = 0; j < 12; j++) {
          smax = fmax(smax, fabs(vr[j + ix0]) + fabs(vr[j + 12 * ki]));
        }
        smax = 1.0 / smax;
        ii = ix0 + 12;
        vectorUB = ((ii - ix0) / 2 * 2 + ix0) + 1;
        ...
        iyend = b_vectorUB - 2;
        for (j = ii + 1; j <= iyend; j += 2) {
          r = _mm_loadu_pd(&vr[j - 1]);
          r = _mm_mul_pd(_mm_set1_pd(smax), r);
          _mm_storeu_pd(&vr[j - 1], r);
        }
        for (j = b_vectorUB; j <= vectorUB; j++) {
          vr[j - 1] *= smax;
        }
      }
    }
