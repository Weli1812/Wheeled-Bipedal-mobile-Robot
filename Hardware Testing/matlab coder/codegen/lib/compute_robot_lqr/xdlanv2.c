    ...
    *c = 0.0;
    ...
    if ((temp == 0.0) && ((*b < 0.0) != (*c < 0.0))) {
      ...
      *sn = 0.0;
      ...
      bcmis = fmin(rt1r, tau) * (double)count * (double)i;
      rt1r = fmax(fabs(p), bcmax);
      z = p / rt1r * p + bcmax / rt1r * bcmis;
      if (z >= 8.8817841970012523E-16) {
        rt1r = sqrt(rt1r) * sqrt(z);
        if (p < 0.0) {
          rt1r = -rt1r;
        }
        z = p + rt1r;
        ...
        *d -= bcmax / z * bcmis;
        ...
        if (tau < bcmis) {
          rt1r = tau / bcmis;
          tau = bcmis * sqrt(rt1r * rt1r + 1.0);
        } else if (tau > bcmis) {
          bcmis /= tau;
          tau *= sqrt(bcmis * bcmis + 1.0);
        ...
        *c = 0.0;
        ...
        if (bcmax < rt1r) {
          bcmis = bcmax / rt1r;
          tau = rt1r * sqrt(bcmis * bcmis + 1.0);
        } else if (bcmax > rt1r) {
          rt1r /= bcmax;
          tau = bcmax * sqrt(rt1r * rt1r + 1.0);
