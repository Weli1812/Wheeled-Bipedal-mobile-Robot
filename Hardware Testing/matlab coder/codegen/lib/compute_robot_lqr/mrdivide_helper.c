...
void mrdiv(creal_T A[36], const creal_T B[36])
{
  ...
  int n;
  signed char ipiv[6];
  memcpy(&b_A[0], &B[0], 36U * sizeof(creal_T));
  for (k = 0; k < 6; k++) {
  ...
  for (j = 0; j < 5; j++) {
    ...
    n = 7 - j;
    kBcol = 0;
    smax = fabs(b_A[jj].re) + fabs(b_A[jj].im);
    for (k = 2; k < n; k++) {
      jAcol = (b + k) - 1;
      s = fabs(b_A[jAcol].re) + fabs(b_A[jAcol].im);
      if (s > smax) {
        kBcol = k - 1;
        smax = s;
      }
