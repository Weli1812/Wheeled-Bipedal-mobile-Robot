    ...
    if (notdone) {
      ...
      do {
        ...
        if (j > 0) {
          ...
          b_ix = 0;
          exitg6 = false;
          while ((!exitg6) && (b_ix <= static_cast<unsigned char>(l) - 1)) {
          ...
          if (skipThisRow) {
            ...
            if (j != l) {
              b_ix = (j - 1) * 12;
              iy = (l - 1) * 12;
              c_ix = static_cast<unsigned char>(l);
              for (int i{0}; i < c_ix; i++) {
                kend = b_ix + i;
                temp = A[kend];
                ix = iy + i;
                A[kend] = A[ix];
        ...
        while ((!exitg6) && (ix <= l)) {
          ...
          if (skipThisRow) {
            ...
            if (ix != ilo) {
              c_ix = (ix - 1) * 12;
              j = (ilo - 1) * 12;
              kend = static_cast<unsigned char>(l);
              ...
              ix = (j + ix) - 1;
              b_ix = (j + ilo) - 1;
              iy = static_cast<unsigned char>(13 - ilo);
              for (int i{0}; i < iy; i++) {
                c_ix = ix + i * 12;
                temp = A[c_ix];
                kend = b_ix + i * 12;
    ...
    while ((!exitg3) && (!skipThisRow)) {
      ...
      do {
        ...
        if (b_i + 1 <= l) {
          ...
          double s;
          b_ix = (l - ilo) + 1;
          c = blas::xnrm2(b_ix, A, b_i * 12 + ilo);
          ix = (ilo - 1) * 12 + b_i;
          j = ix + 1;
          r = 0.0;
          if (b_ix >= 1) {
          ...
          if (13 - ilo < 1) {
            ...
            if (13 - ilo > 1) {
              temp = std::abs(A[ix]);
              for (int i{2}; i <= b_ix; i++) {
                s = std::abs(A[ix + (i - 1) * 12]);
                if (s > temp) {
                  c_ix = i;
                  temp = s;
                }
          ...
          temp = std::abs(A[b_i + 12 * ((c_ix + ilo) - 2)]);
          if ((c == 0.0) || (r == 0.0)) {
            b_i++;
          } else {
            double f;
