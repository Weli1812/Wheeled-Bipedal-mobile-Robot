...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART1)
  {
    // Always invalidate the base 32-byte aligned cache line
    SCB_InvalidateDCache_by_Addr((uint32_t *)uart_rx_buffer, 32);

    // --- STATE 1: Normal 26-byte synchronized reading ---
    if (huart->RxXferSize == 26)
    {
        if (uart_rx_buffer[0] == 0xAA && uart_rx_buffer[1] == 0x55)
        {
            // Packet is perfectly aligned! Extract data.
            float temp_states[6];
            ...
            // Keep listening for the next 26-byte packet
            HAL_UART_Receive_DMA(&huart1, uart_rx_buffer, 26);
        }
        else
        {
            // Packet rejected (Misaligned). Switch to State 2 (Hunting Mode)
            HAL_UART_Receive_DMA(&huart1, uart_rx_buffer, 1);
        }
    }

    // --- STATE 2: Hunting Mode (Reading 1 byte at a time to find headers) ---
    else if (huart->RxXferSize == 1)
    {
        static uint8_t last_byte = 0;
        if (last_byte == 0xAA && uart_rx_buffer[0] == 0x55)
        {
            // Found the sync headers! Switch to State 3.
            // Request the remaining 24 bytes of the payload.
            HAL_UART_Receive_DMA(&huart1, &uart_rx_buffer[2], 24);
            last_byte = 0; // Reset for next time
        }
        else
        {
            // Store this byte and keep hunting
            last_byte = uart_rx_buffer[0];
            HAL_UART_Receive_DMA(&huart1, uart_rx_buffer, 1);
        }
    }

    // --- STATE 3: Payload Recovery (Reading the 24 bytes after headers) ---
    else if (huart->RxXferSize == 24)
    {
        // Payload successfully recovered after a framing error
        float temp_states[6];
        memcpy(temp_states, &uart_rx_buffer[2], 24);
        ...
        // We are fully synchronized again! Go back to State 1
        HAL_UART_Receive_DMA(&huart1, uart_rx_buffer, 26);
    }
  }
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    while (1)
    {
      ...
      // This locks your control loop dt to the exact frequency of your sensors.
      if (data_ready == 1)
      {
        ...
        // Re-enable motors in case watchdog had disabled them
        HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, GPIO_PIN_SET);

        // 1. Run the LQR Control Matrix calculation (now with deadband!)
        Simulink_step();

        // 2. Apply generated PWM to hardware timers dynamically

        // Right Motor (TIM1) -> PA9 and PA10
        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, (uint32_t)rtY.RPWM_R1);
        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, (uint32_t)rtY.RPWM_R);
      ...
      else
      {
        ...
        {
          // Disable motor drivers globally
          HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, GPIO_PIN_RESET);

          // Force duty cycles to 0 to prevent motor spinning
