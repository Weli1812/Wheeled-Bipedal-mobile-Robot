...
namespace coder {
namespace internal {
void mrdiv(float A[42], const float B[36])
{
  float b_A[36];
  float smax;
  int i;
  ...
  for (int j = 0; j < 5; j++) {
    ...
    jA = 7 - j;
    jBcol = 0;
    smax = static_cast<float>(fabs(static_cast<double>(b_A[b_tmp])));
    for (int k = 2; k < jA; k++) {
      float s;
      s = static_cast<float>(fabs(static_cast<double>(b_A[(b_tmp + k) - 1])));
      if (s > smax) {
        jBcol = k - 1;
        smax = s;
      }
