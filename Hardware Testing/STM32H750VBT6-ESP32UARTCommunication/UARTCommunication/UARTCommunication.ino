...
void setup() {
  ...
  // CRITICAL: Extremely short timeout so readStringUntil doesn't block the 200Hz EKF loop!
...
void loop() {
  ...
  if (Serial2.available()) {
    ...
    if (stm32Data.length() > 0) {
      // Print safely without ruining plotters
  ...
  if (currentMicros - previousLocMicros >= 5000) {
    ...
    // =================================================================
    // TASK 3: SEND BINARY ARRAY TO STM32
    // =================================================================
    // Package the floats into the payload array (24 bytes total)
    float payload[6];
    payload[0] = phi_rad;
    ...
    // Write the raw memory bytes directly to Serial2
